...
def build_keypoints(count=16, width=0.25, x_offset=0.3, z_offset=0.25):
    ...
    """
    counts = np.linspace(0, 2 * np.pi, num=count, endpoint=False)
    keypoints = []
    for t in counts:
        y, z = get_lemniscate_keypoint(t, a=width)
        keypoints.append([x_offset, y, z + z_offset])
...
def ik_track(model, data, site_name, target_pos,
    ...
    for i in range(max_iters):
        ...
        b = np.concatenate((pos_gain * err_pos, np.zeros((3,))))
